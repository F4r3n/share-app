...
{#if isSelectable}
    <button
        class="btn my-0.5 p-0 px-1 text-nowrap text-ellipsis select-none rounded-md h-fit w-fit"
...
{:else}
    <span
        class="px-2 mx-0 text-left text-nowrap text-ellipsis select-none font-bold"
        >{channelName}</span
    >
{/if}
